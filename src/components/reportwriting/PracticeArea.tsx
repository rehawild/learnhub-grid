...
import { CheckCircle2, XCircle, RotateCcw, Trophy, ArrowRight, PenTool } from "lucide-react";
    ...
    if (isFinished) {
        return (
            <Card className="w-full max-w-xl mx-auto text-center p-8 space-y-6 animate-in fade-in zoom-in duration-500">
                ...
                <CardHeader>
                    <CardTitle className="text-3xl font-bold">Report Drafted!</CardTitle>
                    <CardDescription className="text-lg">
                        You've mastered the formal tone and structure of professional reports.
                    </CardDescription>
                </CardHeader>
                <CardContent>
                    ...
                    <p className="text-muted-foreground font-medium uppercase tracking-widest text-xs">Writing Score</p>
                </CardContent>
    ...
    return (
        <div className="w-full max-w-3xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="text-center space-y-2">
                ...
                <h2 className="text-2xl font-bold">Synthesize the report sentence</h2>
                <p className="text-muted-foreground italic">Tone Requirement: {currentPhrase.tone.toUpperCase()}</p>
            </div>
            ...
            <Card className="border-2 border-dashed border-primary/30 bg-muted/5 p-6 min-h-[120px] flex flex-wrap gap-2 items-center justify-center">
                {selectedWords.length === 0 && <span className="text-muted-foreground/50 italic font-medium">Reconstruct the formal phrasing...</span>}
                    ...
                    <div className="w-full flex flex-col items-center gap-6">
                        <div className={cn(
                            ...
                            {isCorrect ? (
                                <>
                                    ...
                                    <span className="font-bold text-lg">Professionally articulated.</span>
                                </>
                            ) : (
                                <>
                                    <XCircle className="w-6 h-6" />
                                    <span className="font-bold text-lg">Incorrect phrasing for this section.</span>
                                </>
                            )}
                        </div>
                        ...
                        {!isCorrect && (
                            <div className="text-center px-6 py-4 bg-muted/30 rounded-2xl border border-border/50">
                                <p className="text-sm text-muted-foreground mb-1 uppercase tracking-tighter font-bold">Standard Form:</p>
                        ...
                        >
                            {isCorrect ? (
                                <>Next Section <ArrowRight className="w-5 h-5 ml-2" /></>
                            ) : (
                                <>Revise Sentence <RotateCcw className="w-5 h-5 ml-2" /></>
                            )}
                        </Button>
                    </div>
                )}
