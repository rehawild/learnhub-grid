...
import { ClauseDeck, ClauseState } from "@/types/clausebuilder";
import { toast } from "sonner";

export const useClauseBuilder = () => {
    const [state, setState] = useState<ClauseState>({
        currentDeck: null,
        currentIndex: 0,
        correctCount: 0,
        isComplete: false,
    ...
    const selectDeck = (deck: ClauseDeck) => {
        setState({
            currentDeck: deck,
            currentIndex: 0,
            correctCount: 0,
    ...
    const checkAnswer = (option: string) => {
        ...
        const isCorrect = option === currentQuestion.correctClause;
        ...
        if (isCorrect) {
            toast.success("Correct Combination!", { duration: 1500 });
        } else {
